...
export const renameMCP = async (currentName: string, newName: string) => {
  const spinner = createSpinner(
    `Renaming alias from "${currentName}" to "${newName}"`
  );
  spinner.start();
  ...
  try {
    ...
    if (!config[currentName]) {
      spinner.error(`Alias "${currentName}" not found in configuration.
    ...
    if (config[newName]) {
      spinner.error(`Alias "${newName}" already exists in configuration`);
      return;
    }
    ...
    try {
      ...
      if (processEntry && processEntry.pm2_env.status === "online") {
        ...
        spinner.update(
          `Stopping process "${currentProcessName}" for "${currentName}"`
        );
    ...
    try {
      ...
      if (isProcessRunning) {
        ...
        try {
          ...
          spinner.success(`Process restarted with new name "${newName}"`);
        } catch (startError) {
          spinner.error(
